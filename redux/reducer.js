  ...
  switch (action.type) {
    ...
    case CHANGE_STATUS:
      let newArr = state.tasks.map(task => {
        if (task.slug === action.task.slug)
          task.metafields[0].value = !task.metafields[0].value;
        return task;
      });
...
export const getAllTasks = () => dispatch => {
  axios
    .get(`https://api.cosmicjs.com/v1/your-bucket-slug-name/object-type/tasks`)
    .then(response => {
...
export const postNewTask = task => dispatch => {
  dispatch(
    addTask({
      title: task,
      metafields: [{ value: false }],
      slug: formatSlug(task)
    })
  );
  axios
    .post(`https://api.cosmicjs.com/v1/your-bucket-slug-name/add-object`, {
      type_slug: "tasks",
      title: task,
      content: "New Task",
      metafields: [
...
export const putChangeStatus = (task, bool) => dispatch => {
  ...
  axios
    .put(`https://api.cosmicjs.com/v1/your-bucket-slug-name/edit-object`, {
      slug: task.slug,
      metafields: [
        {
          title: "Is Complete",
...
export const deleteTask = slug => dispatch => {
  ...
  axios
    .delete(`https://api.cosmicjs.com/v1/your-bucket-slug-name/${slug}`)
    .then(response => {
      console.log(response.data);
    })
    .catch(err => {
